...
migrate_site() {
    ...
    if [ $use_cache -eq 1 ]; then
        ...
        if ssh -T -q -o LogLevel=QUIET -p "$SSH_PORT" "$REMOTE_SSH" "exit"; then
            print_success "SSH credentials verified"
        else
            print_error "SSH authentication failed. Please verify your credentials."
            exit 1
        fi
